...
interface Props {
  params: {
    id: string; // This is now the slug of the Otakudesu anime
  };
  searchParams: {
    ep?: string;
    content?: string;
...
export default async function WatchPage({ params, searchParams }: Props) {
  // 1. Fetch anime details directly from Otakudesu using slug (id)
  const anime = await getOtakuAnimeDetail(params.id);
  if (!anime) {
    notFound();
  }
  ...
  const activeEpisodeParam = searchParams.ep || "";
  const currentEpisode = episodes.find((ep) => ep.number === activeEpisodeParam) || episodes[0] || null;

  // 3. Scrape all resolutions / mirrors for the current episode
  let resolutions: any[] = [];
  if (currentEpisode) {
    resolutions = await getEpisodeResolutions(currentEpisode.url);
  }

  // 4. Select the default quality/mirror payload if not defined in query
  ...
  // 5. Resolve embed stream URL using active content token
  let streamUrl = "";
  ...
  const isActiveEpisode = (epNumber: string) => {
    if (activeEpisodeParam) {
      return epNumber === activeEpisodeParam;
    }
    // Default to first episode if no ep param
    return episodes[0] && epNumber === episodes[0].number;
  };
